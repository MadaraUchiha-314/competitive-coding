...
long long update(long long current, long long value) { return value; }

void solve() {
  SegTree<long long, long long, create, combine, update, (long long) 1e9> s(nodes);
  for (auto q: queries) {
    if (q.first == 1) {
      s.update(q.second.first - 1, q.second.second);
    } else {
    cout << s.query(q.second.first - 1, q.second.second - 1) << NEW_LINE;
    }
  }
}
